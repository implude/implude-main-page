...
export default function VisionPage() {
  ...
  const layout498 = useMediaQuery({ query: '(max-width: 498px)' })
  const layout549 = useMediaQuery({ query: '(max-width: 549px)' })

  return (
    <div style={{ padding: '13.75rem 0' }}>
      <Col gap="6.25rem">
        <Row justify="center" gap="4.75rem" style={{flexWrap: 'wrap'}}>
          <Title color="--black" style={{marginLeft: '2rem'}}>비전</Title>
          <Col gap="0.625rem">
            <Row justify={layout498 ? 'center' : 'right'} gap={layout498 ? '2.5rem' : '5.375rem'} align="column">
              <VisionVer color={Version == 0 ? '--black' : '--gray-400'} $bold onClick={Ver1}>
                임플루드 비전 1.0
              </VisionVer>
              <VisionVer color={Version == 0 ? '--gray-400' : '--black'} $bold onClick={Ver2}>
                임플루드 비전 2.0
              </VisionVer>
            </Row>
            <div style={{ width: isMob ? '86vw' : '78vw', height: '0.125rem', background: 'gray' }}></div>
          </Col>
        </Row>
        {RendComponent}
        <Row gap={'3rem'} justify="center" style={{flexWrap: 'wrap'}}>
          <VisionTitle color="--black" style={{textAlign: layout549 ? 'center' : ''}}>핵심 가치</VisionTitle>
      ...
      <Blank height="5.75rem" />
      <Row gap="5.125rem" justify="center" style={{flexWrap: 'wrap'}}>
        <VisionTitle color="--black">인재상</VisionTitle>
        <Row style={{flexWrap: 'wrap'}} justify='center'>
          <Col gap={'1.25rem'} style={{width: layout549 ? '70vw' : ''}}>
            <div>
...
const VisionVer = styled(Header1)`
  font-size: 2.2rem;
  cursor: pointer;
`
