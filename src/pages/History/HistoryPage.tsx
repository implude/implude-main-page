    ...
    <>
      ...
      <Row justify="center">
        <IntroText color={"--gray-black"}>
          <span>임플루드</span>는 오늘도<br/>세상을 바꾸고 있습니다
        </IntroText>
      </Row>
          ...
          <YearContainer gap={"74px"}>
            {[...Array(9)].map((v, i)=>(
              ...
              <Row width="120px" justify="space-between">
                <Year hover={{color:"--gray-black"}} enabled={2023-i === activated}  onClick={()=>setActivated(2023-i)}>{2023-i}</Year>
          ...
          <HistoryContainer gap={"100px"}>
              ...
              {data.map((v, i)=>(
                <HistoryBlock onClick={()=>setActivated(2023-i)}>
                  <Header1 $bold color={"--gray-400"}>{v.date}</Header1>
                  <Header1 $bold  color={"--gray-black"}>{v.text}</Header1>
                </HistoryBlock>                
              ))}
          </HistoryContainer>        
        </Row>        
